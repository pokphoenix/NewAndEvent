...
<style></style>
@endsection @section('module_name', 'COMPANIES>COMPANYGROUP') @section('page_name', 'CREATE') @section('body')
<div class="x_content">
	<div class="context">
		{{-- Error --}} @if ($errors->any())
		...
		@endif
		<form action="{{ route('backOffice.setting.company-group.store') }}" method="POST" style="width: 80%; margin: 0 auto;" id="form-data">
			{{ csrf_field() }}
			<section>
				...
				<div class="row">
					<div class="col-xs-12 col-sm-12">
						<div class="form-group">
							...
							<input type="text" id="txt-name" class="form-control" name="name" value="{{(isset($data['duplicate']) ? $data['duplicate']->name : old('name'))}}">
						</div>
					</div>
					<div class="col-xs-12 col-sm-12">
						<div class="form-group">
							<select name="customer_tier_id" id="customer_tier_id" class="form-control">
								...
								<option value="{{ $customertier->id }}" @if((isset($data[ 'duplicate']) && $data[ 'duplicate']->customer_tier_id == $customertier->id) || old( 'customer_tier_id')==$customertier->id) selected="selected" @endif>{{
									$customertier->name }}
								</option>
								@endforeach
							</select>
					...
					<div class="col-xs-12 col-sm-12">
						<div class="form-group">
							...
							<textarea class="form-control" rows="5" name="description" id="txt-desc">{{ (isset($data['duplicate']) ? $data['duplicate']->description : old('description'))}}</textarea>
						</div>
					</div>
					<div class="col-xs-12 col-sm-12">
			...
			<section>
				<div class="row" id="select-company">
					...
					<?php if(is_array(old( 'company_id')) && in_array($company->id,old('company_id'))): ?>
...
<script type="text/javascript">
	<?php 
				...
				foreach($data['company'] as $company){
					echo '{id:"'.$company->id.'",show:'.(is_array(old( 'company_id')) && in_array($company->id,old('company_id')) ? "false" : "true").',name:"'.$company->name.'",desc:"No. 18, kijipanich Building, Room 405, 4th Floor Patpong Road, Suriyawong Bangrak Bangkok 10520"},';
				}
				echo '];';
				?>
