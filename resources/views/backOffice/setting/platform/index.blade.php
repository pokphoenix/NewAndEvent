...
@section('head')
    <link rel="stylesheet" type="text/css" href="{{ asset('/css/back-office/setting/platform/index.css')}}"/>
@endsection
          ...
          <tr>
            <td class="col-1">
              <div class="col-1-content" >
                ...
                <div class="form-group">
                  <button type="button" class="btn btn-default-background" onclick="window.location='{{ url("back-office/setting/platform/create") }}'">
            ...
            <td class="col-3">
              ...
              <div class="text-right col-3-content">
                ...
                <form action="{{route('backOffice.setting.platform.excel')}}" target="_blank" method="POST">
                      {!! csrf_field() !!}
                  <div class="form-group">
                ...
                <form action="{{route('backOffice.setting.platform.delete-all')}}" method="POST">
                    {!! csrf_field() !!}
                    <div class="form-group">
  ...
  <div class="platform-table">
    ...
    <div class="body-table scroll-2">
      <table class="table ngin-table">
        <tbody>
          ...
          <tr class="@if ( $p->trashed() ) tr-trash @endif">
            <td class="blank">
              <div class="form-group">
                  <input type="checkbox" name="chkItem[]" class="iCheck item" UseForce="@if($p->deleted_at) true @endif" value="{{$p->id}}">
              </div>
      			</td>
      			<td class="name">
              <p>{{ $p->name }}</p>
              ...
              <div class="buttons">
                @if( $p->trashed() )
                  <form action="{{route('backOffice.setting.platform.restore',$p->id)}}" method="POST">
                    {!! csrf_field() !!}    
                      {!! method_field('patch') !!}
                  ...
                  <form method="POST" action="{{ route('backOffice.setting.platform.destroy', $p->id) }}">
                      {{ csrf_field() }}
                      {{ method_field('DELETE') }}
                  ...
                  <button type="button" class="btn btn-ngin btn-default" onclick="window.location='{{ url("back-office/setting/platform/". $p->id ."/edit") }}'">
                    <span class="btn-label">
                      <i class="fa fa-pencil-square-o success" aria-hidden="true"></i>
                    </span>Edit
                  </button>
                  <button type="button" class="btn btn-ngin btn-default" onclick="window.location='{{ route('backOffice.setting.platform.duplicate', $p->id) }}'">
                    <span class="btn-label">
    ...
    <div class="row page-showing-pagination">
      ...
      <div class="col-xs-6 showing">
        Showing {{ $platforms->firstItem() }}-{{ $platforms->lastItem() }} of {{ $platforms->total() }}
      </div>
...
@section('script')
<script src="{{ asset('js/back-office/setting/platform/index.js') }}"></script>
