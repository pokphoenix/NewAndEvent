...
@section('head')
<link rel="stylesheet" type="text/css" href="{{ asset('/css/back-office/setting/publisher/create.css')}}"/>
@endsection
  ...
  <form action="{{ $action }}" method="POST">
    ...
    <div class="wrapForm">
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="name" id="name" value="{{ ( isset($publisher) ) ? $publisher->name : '' }}">
        </div>
        <div class="form-group">
            <label for="exampleInputPassword1">DESCRIPTION:</label>
            <textarea class="form-control" rows="3" name="description" id="description">{{ ( isset($publisher) ) ? $publisher->description : '' }}</textarea>
        </div>
    </div>
    <div class="line"></div>
...
@section('script')
<script src="{{ asset('js/back-office/setting/publisher/create.js') }}"></script>
@endsection
